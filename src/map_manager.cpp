...
void MapManager::addMapPoint(const cv::Scalar &color)
{
    ...
    if( plm->isobs_ ) {
        colored_pt = pcl::PointXYZRGB((unsigned char)255, (unsigned char)0, (unsigned char)0);
    } else {
        colored_pt = pcl::PointXYZRGB((unsigned char)plm->color_[0]
                                    , (unsigned char)plm->color_[0]
                                    , (unsigned char)plm->color_[0]
                                    );
    }
    colored_pt.x = 0.;
...
void MapManager::updateMapPoint(const int lmid, const Eigen::Vector3d &wpt, const double kfanch_invdepth)
{
    ...
    if(plmit->second->isobs_ ) {
        ...
        colored_pt = pcl::PointXYZRGB((unsigned char)plmit->second->color_[0]
                                    , (unsigned char)plmit->second->color_[0]
                                    , (unsigned char)plmit->second->color_[0]
                                    );
    }
    colored_pt.x = wpt.x();
...
void MapManager::mergeMapPoints(const int prevlmid, const int newlmid)
{
    ...
    // Visualization related part for pointcloud obs
    pcl::PointXYZRGB colored_pt;
    colored_pt = pcl::PointXYZRGB((unsigned char)0, (unsigned char)0, (unsigned char)0);
    colored_pt.x = 0.;
    colored_pt.y = 0.;
    colored_pt.z = 0.;
...
bool MapManager::setMapPointObs(const int lmid) 
{
    ...
    // Update MP color
    colored_pt = pcl::PointXYZRGB((unsigned char)200, (unsigned char)0, (unsigned char)0);
    colored_pt.x = pcloud_->points.at(lmid).x;
    colored_pt.y = pcloud_->points.at(lmid).y;
    colored_pt.z = pcloud_->points.at(lmid).z;
    pcloud_->points.at(lmid) = colored_pt;
